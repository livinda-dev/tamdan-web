...
export async function GET(req: NextRequest) {
    try {
        ...
        if (questionError) {
            console.error("[Entries API] get user_question error:", questionError.message);
...
export async function POST(req: NextRequest) {
  try {
    ...
    // Insert into public.user_question with FK to public.user(id)
    const insertPayload = {
      question: content,
      user_id: userRow.id,
    } as const;

    const { data, error } = await supabase.from("user_question").insert(insertPayload).select();

    if(error){
        console.error("[Entries API] insert into user_question error:", error.message);
