...
export async function POST(req: NextRequest) {
  try {
    ...
    const { data: existingTitle, error: chckError } = await supabase.from("topics").select("id,topic").eq("user_id", userRow.id).maybeSingle();
    ...
    let error;
    if (existingTitle) {
      const updateResult = await supabase.from("topics").update({ topic: content }).eq("user_id", userRow.id).select();
